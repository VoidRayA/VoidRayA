...
- **주요 기능**:  
  - 보호자 대시보드 (건강 데이터 시각화, 날씨, 일정 관리)  
  - 병원 추천 및 경로 안내 기능  
  - JWT 기반 사용자 인증/인가  
- **본인 역할**:  
  - 프론트엔드 UI/차트 개발 (React + Chart.js)  
  - KakaoMap/Tmap API 연동, 병원 경로 표시 로직 구현  
  - DB 테이블 설계 일부 및 API 연동 테스트  
...
- **업무 내용**:  
  - 기존 ERP 모듈 유지보수 및 오류 수정  
  - 신규 기능 추가 (업무 효율화 기능, 데이터 조회 화면 등)  
  - 사용자 요청사항 반영 및 배포 지원  
- **성과**:  
  - 반복 작업 자동화로 업무 효율 개선  
  - 유지보수 프로세스 개선 및 사용자 만족도 향상  
...
- **主な機能**:  
  - 保護者ダッシュボード（健康データ可視化、天気、スケジュール管理）  
  - 病院推薦および経路案内  
  - JWTベースのユーザー認証・認可  
- **担当範囲**:  
  - フロントエンドUI/チャート開発（React + Chart.js）  
  - KakaoMap/Tmap API連携、病院経路表示ロジック実装  
  - DBテーブル設計一部およびAPI連携テスト  
...
- **業務内容**:  
  - 既存ERPモジュールの保守および不具合修正  
  - 新規機能追加（業務効率化機能、データ照会画面など）  
  - ユーザー要望の反映およびリリースサポート  
- **成果**:  
  - 繰り返し作業の自動化による業務効率向上  
  - 保守プロセス改善およびユーザー満足度向上  
